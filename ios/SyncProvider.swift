...
///
/// We name the Swift type `HybridSyncProvider` (not `SyncProvider`) to avoid
/// a symbol collision with the C++ `SyncProvider` class generated by Nitro
/// inside the `margelo::nitro::syncprovider` namespace. The Nitro `iosModuleName`
/// (`SyncProvider`) and the logical module name used from JS
/// (`NitroModules.createHybridObject<SyncProvider>('SyncProvider')`) remain
/// unchanged — only the underlying Swift class name is different.
///
...
public final class HybridSyncProvider: HybridSyncProviderSpec {
    // MARK: - Components
    ...
    public override init() {
        ...
        HybridSyncProvider.weakInstance = self
    ...
    public func getSyncConfig() throws -> Promise<SyncOptions> {
        return Promise.async { [weak self] in
            ...
            let snapshot = self.configLock.withLock {
                self.lastSyncOptions
            }
    ...
    @objc public static func handleBackgroundURLSessionEvents(
        ...
        if let live = HybridSyncProvider.weakInstance {
    ...
    fileprivate static weak var weakInstance: HybridSyncProvider?
}

// MARK: - Translation helpers

extension HybridSyncProvider {
