...
- **`useSyncSnapshot` hook (A1).** New read-only dashboard aggregator that combines connection, queue, and sync-engine state into a single reactive snapshot driven by **one** `addSyncEventListener` subscription. Unlike [`useOfflineQueue`](https://gabriel-sisjr.github.io/react-native-sync-provider/docs/api-reference/hooks/useOfflineQueue) it exposes **no** action methods — it is a pure observer for status dashboards, badges, and "what is the sync engine doing right now?" UIs. `UseSyncSnapshotResult` surfaces `connection`, `size`, `items`, `isSyncing`, `isPaused`, `progress`, `lastResult`, `isWaitingForConnection`, and `error`. The single callback batches all `setState` calls into one React 18 render, guards every async update with a `cancelledRef`, and removes the listener on unmount; it is safe to call before the native module loads (returns inert state, never throws on render). Exported from `src/index.tsx` (with an inert `src/index.web.tsx` stub) and re-exported via `src/hooks/index.ts`.
- **Forward-looking Block B (v0.3) types.** `NetworkQuality` (`OFFLINE` → `POOR` → `MODERATE` → `GOOD` → `EXCELLENT`, ordered worst → best) and `SyncDeadLetterItem` (a `SyncItem` plus `attempts`, `lastAttemptAt`, `lastErrorCode`, `lastErrorMessage`, `deadLetteredAt`) are introduced now so consumer types can depend on their final shape. They are surfaced **only** as the optional `UseSyncSnapshotResult.networkQuality` and `UseSyncSnapshotResult.deadLetter` fields and are **ALWAYS `undefined` in v0.2** — real values arrive with the v0.3 network-quality classifier (Gap #2) and dead-letter store (Gap #4).
